...
function Card({c}){
  if(c==="|")return<span style={{fontSize:10,color:C.text3,padding:"0 4px",alignSelf:"center"}}>|</span>;
  ...
  return(
    <div style={{display:"inline-flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
      width:46,height:58,borderRadius:8,background:"#fff",
      border:`2px solid ${red?"#fca5a5":C.border2}`,fontWeight:800,userSelect:"none",flexShrink:0}}>
      <span style={{fontSize:18,lineHeight:1,color:red?"#dc2626":"#111827"}}>{rank}</span>
      <span style={{fontSize:14,lineHeight:1,marginTop:2,color:red?"#dc2626":"#111827"}}>{suit}</span>
    </div>
  );
}

function Cards({txt,board,calle}){
  let cards=[];
  if(board){
  ...
  return<div style={{display:"flex",flexWrap:"wrap",gap:5,marginTop:8}}>{cards.map((c,i)=><Card key={i} c={c}/>)}</div>;
}

function Timeline({seq,calle}){
  ...
  parts.forEach(p=>{
    ...
    if(!pl.includes("flop:")&&!pl.includes("turn:")&&!pl.includes("river:"))actions["Preflop"]=p.replace(/preflop:/i,"").trim().slice(0,32);
    if(pl.includes("flop:"))actions["Flop"]=p.replace(/flop:/i,"").trim().slice(0,32);
    if(pl.includes("turn:"))actions["Turn"]=p.replace(/turn:/i,"").trim().slice(0,32);
    if(pl.includes("river:"))actions["River"]=p.replace(/river:/i,"").trim().slice(0,32);
  });
  return(
    <div style={{display:"flex",gap:0,overflowX:"auto",paddingBottom:4,marginBottom:16}}>
      {streets.map((s,i)=>{
        const active=i===activeIdx;
        return(
          <div key={s} style={{display:"flex",flexDirection:"column",alignItems:"center",minWidth:72,position:"relative",flex:1}}>
            {i<3&&<div style={{position:"absolute",top:14,left:"50%",width:"100%",height:1,background:C.border,zIndex:0}}/>}
            ...
            <div style={{fontSize:10,color:active?C.blueTxt:C.text2,marginTop:5,fontWeight:600}}>{s}</div>
            {actions[s]&&<div style={{fontSize:9,color:C.text3,marginTop:2,textAlign:"center",maxWidth:80,lineHeight:1.3}}>{actions[s]}</div>}
          </div>
        );
      })}
    </div>
...
const SL=(txt)=><div style={{fontSize:10,color:C.text2,textTransform:"uppercase",letterSpacing:".06em",fontWeight:600,marginBottom:8}}>{txt}</div>;
const DIV=<div style={{borderTop:`1px solid ${C.border}`,margin:"14px 0"}}/>;
...
export default function App(){
  ...
  function nextSpot(p=pool){
    if(!p.length)return;
    setSpot(p[Math.floor(Math.random()*p.length)]);
    setChosen(null);setEvaled(false);
  }

  function evaluate(){
    if(!chosen||evaled||!spot)return;
    setEvaled(true);
  ...
  const selStyle={fontSize:14,padding:"8px 10px",borderRadius:8,border:`1px solid ${C.border2}`,background:C.bg2,color:C.text,width:"100%",WebkitAppearance:"none"};
  const btnStyle=(bg,color,border)=>({padding:"10px 20px",borderRadius:8,border:`1px solid ${border||C.border2}`,background:bg,color,fontSize:14,cursor:"pointer",fontWeight:500,fontFamily:"inherit"});

  if(loading)return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:C.bg,color:C.text2,gap:12}}>
      <div style={{fontSize:40}}>♠</div>
      <div style={{fontSize:14}}>Cargando spots…</div>
    </div>
  );
  ...
  if(error)return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:C.bg,padding:24,gap:16}}>
      <div style={{background:C.redBg,color:C.redTxt,padding:"12px 16px",borderRadius:10,fontSize:14,textAlign:"center"}}>{error}</div>
      <button onClick={loadSpots} style={btnStyle(C.bg2,C.text,C.border2)}>Reintentar</button>
    </div>
  );

  // ── PANEL IZQUIERDO: controles + tracker ──
  const LeftPanel=(
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      {/* Header */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <span style={{fontSize:isDesktop?22:20,fontWeight:700}}>♠ Poker Trainer</span>
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          <span style={{fontSize:11,padding:"3px 10px",borderRadius:99,background:C.bg2,border:`1px solid ${C.border}`,color:C.text2}}>{pool.length}/{spots.length}</span>
          <button onClick={loadSpots} title="Actualizar" style={{fontSize:18,background:"none",border:"none",color:C.text2,cursor:"pointer",padding:"4px 6px"}}>↺</button>
        </div>
      </div>

      {lastLoaded&&<div style={{fontSize:11,color:C.text3}}>Actualizado: {lastLoaded}</div>}

      {/* Metrics */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
        {[["Resp.",stats.total,C.text],["✓",stats.ok,C.green],["✗",stats.ko,C.red],["Acc.",acc!==null?acc+"%":"—",C.blue]].map(([l,v,c])=>(
          <div key={l} style={{background:C.bg2,borderRadius:10,padding:"10px 8px",textAlign:"center",border:`1px solid ${C.border}`}}>
            <div style={{fontSize:10,color:C.text2,marginBottom:4}}>{l}</div>
            <div style={{fontSize:isDesktop?22:18,fontWeight:700,color:c}}>{v}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
        <div>
          <div style={{fontSize:11,color:C.text2,marginBottom:4}}>Bloque</div>
        ...
        <div>
          <div style={{fontSize:11,color:C.text2,marginBottom:4}}>Calle</div>
      ...
      <div>
        <div style={{fontSize:11,color:C.text2,marginBottom:4}}>Concepto</div>
      ...
      <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
        <span style={{fontSize:11,color:C.text2}}>Rival:</span>
        {["Base teórica","Regular","Recreacional"].map(r=>(
          <button key={r} onClick={()=>setRival(r)} style={{padding:"5px 12px",borderRadius:99,fontSize:12,cursor:"pointer",fontFamily:"inherit",
      ...
      {/* Tracker — solo desktop lo muestra siempre */}
      {stats.total>0&&(
        <div>
          {!isDesktop&&(
            <button onClick={()=>setShowTracker(!showTracker)} style={{...btnStyle(C.bg2,C.text2,C.border),width:"100%",marginBottom:12,textAlign:"center"}}>
              {showTracker?"▲ Ocultar stats":"▼ Ver estadísticas"}
            </button>
          )}
          {(isDesktop||showTracker)&&(
            <div style={{background:C.bg2,border:`1px solid ${C.border}`,borderRadius:14,padding:16}}>
              <div style={{fontSize:13,fontWeight:600,marginBottom:10}}>Por calle</div>
                ...
                return(
                  <div key={k} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:`1px solid ${C.border}`}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      <span style={{fontSize:12,color:C.text2}}>{v.ok}/{v.n}</span>
                      <div style={{width:60,height:5,background:C.bg3,borderRadius:3,overflow:"hidden"}}>
                        <div style={{width:a+"%",height:"100%",background:col,borderRadius:3}}/>
                      </div>
                      <span style={{fontSize:12,color:col,fontWeight:600,minWidth:32}}>{a}%</span>
                    </div>
                  </div>
                );
              })}
              {worst.length>0&&(
                <>
                  <div style={{fontSize:13,fontWeight:600,marginTop:16,marginBottom:10}}>Leaks con más errores</div>
                  {worst.map(([k,v])=>(
                    <div key={k} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:`1px solid ${C.border}`}}>
                      <span style={{fontSize:12,color:C.text2,flex:1,paddingRight:8}}>{k}</span>
                      <span style={{fontSize:12,color:C.red,fontWeight:600}}>{Math.round(v.ok/v.n*100)}%</span>
                    </div>
                  ))}
                </>
              )}
  ...
  const RightPanel=(
    <div>
      {spot?(
        <div style={{background:C.bg2,border:`1px solid ${C.border}`,borderRadius:14,padding:16}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px 16px",marginBottom:14}}>
            {[["Tema",spot.tema],["Concepto",spot.conc],["Posición",`${spot.hero} vs ${spot.vill} · ${spot.stacks}`],["Calle",spot.calle]].map(([l,v])=>(
              <div key={l}>
                <div style={{fontSize:10,color:C.text3,textTransform:"uppercase",letterSpacing:".05em",marginBottom:2}}>{l}</div>
          ...
          {DIV}
          {SL("Secuencia")}
          <Timeline seq={spot.seq} calle={spot.calle}/>
          {DIV}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
            <div>{SL("Board")}<Cards txt={spot.board} board calle={spot.calle}/></div>
            <div>{SL("Tu mano")}<Cards txt={spot.hand}/></div>
          </div>
          {spot.leaks.length>0&&(
            <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:14}}>
              {spot.leaks.map(l=><span key={l} style={{fontSize:10,padding:"3px 8px",borderRadius:99,background:C.leak,color:C.leakTxt,fontWeight:600}}>{l}</span>)}
            </div>
          )}
          {DIV}
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:16}}>
            {getOpts(spot).map(o=>{
              ...
              return(
                <button key={o} onClick={()=>!evaled&&setChosen(o)} style={{padding:"10px 18px",borderRadius:8,border:`${bw} solid ${border}`,background:bg,color,fontSize:14,cursor:evaled?"default":"pointer",fontWeight:500,fontFamily:"inherit"}}>{o}</button>
              );
            })}
          </div>
          {evaled&&(
            <>
              <div style={{padding:"12px 14px",borderRadius:10,fontSize:13,lineHeight:1.6,marginBottom:10,
              ...
              <div style={{background:C.bg3,borderRadius:10,padding:"12px 14px",fontSize:13,lineHeight:1.6,marginBottom:12,color:C.text2,border:`1px solid ${C.border}`}}>{spot.el}</div>
            </>
          )}
        ...
        <div style={{padding:"2rem",textAlign:"center",color:C.text2,fontSize:14}}>No hay spots con esa configuración.</div>
      )}
    </div>
  );
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif"}}>
      {isDesktop?(
        ...
        <div style={{maxWidth:1200,margin:"0 auto",padding:"24px 32px 40px",display:"grid",gridTemplateColumns:"340px 1fr",gap:32,alignItems:"start"}}>
          <div style={{position:"sticky",top:24}}>{LeftPanel}</div>
          {RightPanel}
        </div>
      ):(
        // MÓVIL: una columna
