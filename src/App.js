...
function parseCards(txt) {
  return txt ? txt.trim().split(/\s+/).filter(Boolean) : [];
}

function Card({ c }) {
  if (c === "|") return <span style={{ fontSize: 10, color: C.text3, padding: "0 3px", alignSelf: "center" }}>|</span>;
  ...
  return (
    <div style={{
      ...
      background: "#fff",
      border: `2px solid ${red ? "#fca5a5" : C.border2}`,
      fontWeight: 800, userSelect: "none", flexShrink: 0,
    }}>
...
function Cards({ txt, board, calle }) {
  ...
  if (board) {
    ...
    show.forEach((seg, i) => {
      if (i > 0) cards.push("|");
      parseCards(seg).forEach(c => cards.push(c));
    });
  } else {
    cards = parseCards(txt);
  }
  return (
...
function Timeline({ seq, calle }) {
  ...
  parts.forEach(p => {
    ...
    if (!pl.includes("flop:") && !pl.includes("turn:") && !pl.includes("river:"))
      actions["Preflop"] = p.replace(/preflop:/i, "").trim().slice(0, 32);
        ...
        return (
          <div key={s} style={{ display: "flex", flexDirection: "column", alignItems: "center", minWidth: 72, position: "relative", flex: 1 }}>
            ...
            <div style={{
              ...
              background: active ? C.blue : done ? C.bg3 : C.bg2,
...
const S = (txt, style = {}) => <span style={{ fontSize: 10, color: C.text2, textTransform: "uppercase", letterSpacing: ".06em", fontWeight: 600, marginBottom: 8, display: "block", ...style }}>{txt}</span>;
...
export default function App() {
  ...
  function reset() {
    setStats({ total: 0, ok: 0, ko: 0 }); setTrC({}); setTrL({});
    nextSpot();
  }
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "16px 16px 40px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <span style={{ fontSize: 20, fontWeight: 700, letterSpacing: "-.01em" }}>♠ Poker Trainer</span>
        ...
        {lastLoaded && <div style={{ fontSize: 11, color: C.text3, marginBottom: 16 }}>Actualizado: {lastLoaded}</div>}
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 20, flexWrap: "wrap" }}>
          ...
          {["Base teórica", "Regular", "Recreacional"].map(r => (
            <button key={r} onClick={() => setRival(r)} style={{
              ...
              color: rival === r ? C.blueTxt : C.text2,
            }}>{r}</button>
          ))}
        </div>
        ...
        {spot ? (
          <div style={{ background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 14, padding: 16, marginBottom: 16 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
              <div>{S("Board")}<Cards txt={spot.board} board calle={spot.calle} /></div>
              <div>{S("Tu mano")}<Cards txt={spot.hand} /></div>
            </div>
            ...
            {spot.leaks.length > 0 && (
              <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 14 }}>
                {spot.leaks.map(l => (
                  <span key={l} style={{ fontSize: 10, padding: "3px 8px", borderRadius: 99, background: C.leak, color: C.leakTxt, fontWeight: 600 }}>{l}</span>
                ))}
              </div>
            )}

            {DIV}
            {S("¿Cuál es tu decisión?")}
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 16 }}>
              {getOpts(spot).map(o => {
                ...
                if (evaled) {
                  ...
                  bg = C.blueBg; border = C.blue; color = C.blueTxt; bw = "2px";
                }
                return (
                  <button key={o} onClick={() => !evaled && setChosen(o)} style={{
                    padding: "10px 18px", borderRadius: 8, border: `${bw} solid ${border}`,
                    background: bg, color, fontSize: 14, cursor: evaled ? "default" : "pointer", fontWeight: 500, fontFamily: "inherit"
                  }}>{o}</button>
                );
              })}
            </div>
            ...
            {evaled && (
              <>
                <div style={{
                  padding: "12px 14px", borderRadius: 10, fontSize: 13, lineHeight: 1.6, marginBottom: 10,
        ...
        {stats.total > 0 && (
          <div>
            <button onClick={() => setShowTracker(!showTracker)} style={{ ...btnStyle(C.bg2, C.text2, C.border), width: "100%", marginBottom: 12, textAlign: "center" }}>
              {showTracker ? "▲ Ocultar estadísticas" : "▼ Ver estadísticas de sesión"}
            </button>
            {showTracker && (
